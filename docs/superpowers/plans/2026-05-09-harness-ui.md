...
|---|---|---|---|---|---|---|---|
| H-UI-001 | UI | Sub-Iteration 4 | `web/src/pages/Harness.tsx`; `web/src/components/HarnessAssetEditor.tsx`; `web/src/lib/api.ts`; `web/src/lib/types.ts` | `web/src/test/HarnessPage.test.tsx`; `tests/test_harness_ui_contract.py` | `cd web && npm run test -- HarnessPage.test.tsx`; `.venv/bin/python -m pytest tests/test_harness_ui_contract.py -q` | pending | Expected: UI save payload only contains API-provided Harness file paths under `.ai/harness.yaml` or `.ai/harness/**`; no free-form repo path input is rendered; contract test proves Harness API paths are project-scoped. |
| H-UI-002 | UI | Sub-Iteration 4 | `web/src/pages/Harness.tsx`; `web/src/components/HarnessConflictDialog.tsx`; `web/src/lib/api.ts`; `web/src/lib/types.ts` | `web/src/test/HarnessPage.test.tsx`; `web/scripts/playwright-harness-ui-smoke.mjs` | `cd web && npm run test -- HarnessPage.test.tsx`; `cd web && node scripts/playwright-harness-ui-smoke.mjs` | pending | Expected: save calls validate first, then PUT includes the last read `manifest_hash`; mocked `409 manifest_conflict` displays changed files and does not issue a second PUT. |
| H-UI-003 | UI | Sub-Iteration 4 | `web/src/components/HarnessAssetEditor.tsx`; `web/src/components/HarnessReportPanel.tsx`; `web/src/components/MarkdownViewer.tsx` | `web/src/test/HarnessPage.test.tsx`; `web/src/test/HarnessReportPanel.test.tsx`; `web/src/test/MarkdownViewer.test.tsx` | `cd web && npm run test -- HarnessPage.test.tsx HarnessReportPanel.test.tsx MarkdownViewer.test.tsx` | pending | Expected: malicious Markdown in rules, skills, and report evidence renders without `<script>`, inline event handlers, or unsafe links; Harness components use sanitized Markdown renderer only. |
| H-UI-004 | UI | Sub-Iteration 4 | `web/src/pages/Harness.tsx`; `web/src/components/HarnessAssetEditor.tsx`; `web/src/components/HarnessTaskBoard.tsx`; `web/src/lib/types.ts` | `web/src/test/HarnessPage.test.tsx`; `web/scripts/playwright-harness-ui-smoke.mjs` | `cd web && npm run test -- HarnessPage.test.tsx`; `cd web && node scripts/playwright-harness-ui-smoke.mjs` | pending | Expected: when `permissions.can_edit=false`, edit/save/add/delete controls are absent from DOM, not merely disabled; read-only content remains visible if `can_view=true`. |
| H-REPORT-002 | UI | Harness Report Contract | `web/src/pages/RunDetail.tsx`; `web/src/components/HarnessReportPanel.tsx`; `web/src/lib/api.ts`; `web/src/lib/types.ts`; `web/src/lib/harnessSchema.ts` | `web/src/test/HarnessReportPanel.test.tsx`; `web/scripts/playwright-harness-ui-smoke.mjs` | `cd web && npm run test -- HarnessReportPanel.test.tsx`; `cd web && node scripts/playwright-harness-ui-smoke.mjs` | pending | Expected: warning checks are visible as non-blocking warnings; blocking failures, baseline changes, and evidence refs are shown in RunDetail without changing pipeline status in UI. |
| H-DOD-001 | All | Definition Of Done | `docs/superpowers/plans/2026-05-09-harness-ui.md`; final report for UI iteration | `tests/test_harness_ui_contract.py`; `web/src/test/HarnessPage.test.tsx`; `web/src/test/HarnessReportPanel.test.tsx`; Playwright smoke script | `rg -n "H-UI-001|H-UI-002|H-UI-003|H-UI-004|H-REPORT-002|H-DOD-001|H-DOD-002" docs/superpowers/plans/2026-05-09-harness-ui.md` | pending | Expected: implementation final report copies this matrix, updates every in-scope row with concrete changed files, fresh command output, and evidence. |
| H-DOD-002 | All | Definition Of Done | UI final report; command logs from implementation session | Same as all rows above | `.venv/bin/python -m unittest discover -s tests -v`; `.venv/bin/python -m pytest tests/test_harness*.py tests/test_routes.py -q`; `cd web && npm run test`; `cd web && npm run build`; `cd web && node scripts/playwright-harness-ui-smoke.mjs`; `git diff --check` | pending | Expected: final status cannot be `complete` unless every command has fresh output and every in-scope row has evidence. |
