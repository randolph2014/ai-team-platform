...
| H-QG-002 | verified | `engine/harness_checks.py`; `engine/quality_gates.py` | `tests/test_harness_checks.py` | `! rg -n "subprocess" engine/harness_checks.py && ! rg -n "os\\.system" engine/harness_checks.py && ! rg -n "create_subprocess" engine/harness_checks.py` | Harness Checks has no shell execution import; command execution remains in shared `engine.quality_gates._run_command`. |
...
- Remaining governance placeholders: no.
